...
class tx_bitsoffice_renderTree extends t3lib_extobjbase {
	...
	****************************************************************************/
	 ...
	 */
	function renderTree(&$data, $pageId, &$refArray)	{
			...
			foreach ($refArray as $key => $val) {
				...
				if (!empty($refID) && !empty($val['content'])) {
					foreach ($val['content'] as $ckey => $cTitle) {
						...
						// have to change the hard coded reference style in future.
						
						while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
							$row['bodytext'] = str_replace("---<".trim($key).">---", "<a href=index.php?id=".$this->importID."&type=160&action=getTopic&uid=".$refID." rel='cross-ref' ext:cls=".$refArray[$key]['title'].">".$refArray[$key]['title']."</a>", $row['bodytext']);
