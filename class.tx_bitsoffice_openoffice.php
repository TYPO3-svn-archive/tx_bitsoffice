...
class tx_bitsoffice_openoffice extends tx_bitsoffice_xml {
	...
	function mainWriter($fileName, $conf, $pageId) {
        ...
        if (count($files))    {
				...
				if ($XML_content)    {
					...
					return  $this->pageTreeObj->renderTree ($res, $pageId, $this->refArray);
            ...
            if ( !empty($v) ) {
                ...
                case 'TEXT:ORDERED-LIST':
						...
						if (is_array($v['subTags']))    {
								...
								$tempArr = array();        //samples the <li> elements
                    ...
                    case 'TEXT:REFERENCE-REF':
                        ...
                        if (t3lib_div::inList('complete,cdata',$subV['type']))    {
                            ...
                            if (!empty($this->contentArray ['heading'])) {
                                ...
                                //$this->refArray[$this->getParagraphContent($subV)][] = $this->contentArray ['heading'];

                                

                                //$this->refArray[$this->contentArray ['heading_id']]
										...
										if (is_array($imgData))    {
											...
											if (is_array($imgInfo))    {
													...
													if ($imgInfo[0]>$maxW)    {
														...
														//$imgData = '<a href="#" onclick="'.htmlspecialchars('vHWin=window.open(\''.$writefile.'\',\'_NEW_IMG_WINDOW\',\'width='.($imgInfo[0]+40).',height='.($imgInfo[1]+40).',status=0,menubar=0,scrollbars=1,resizable=1\');vHWin.focus();return false;').'"><img src="'.$writefile.'" class="'.$imgClass.'" width="'.$maxW.'" height="'.floor($imgInfo[1]/($imgInfo[0]/$maxW)).'" style="border: 1px solid black;" border="0" title="Click to open '.($imgInfo[0].'x'.$imgInfo[1]).' pixel window." alt="" /></a><br />'; 
			
														$imgData = '<a href="'.$writefile.'" ><img src="'.$writefile.'" class="'.$imgClass.'" width="'.$maxW.'" height="'.floor($imgInfo[1]/($imgInfo[0]/$maxW)).'" style="border: 1px solid black;" border="0" title="Click to open '.($imgInfo[0].'x'.$imgInfo[1]).' pixel window." alt="" /></a><br />'; 
														//onclick="'.htmlspecialchars('showWindow(\''.$writefile.'\',\').'">
			
														$content.=$this->cObj->stdWrap($imgData,$this->officeConf['imgWrap.']);

													} else {
			
														$imgData ='<img src="'.$writefile.'" class="'.$imgClass.'" width="'.$imgInfo[0].'" height="'.$imgInfo[1].'"'.($imgInfo[0]>50 && $imgInfo[1]>50 ? ' style="border: 1px solid black;"':'').' /><br />'; 
	...
	function getImgData ($subV) {
			...
			$imgData = $this->unzipObj->getFileFromXML(substr($subV['attributes']['XLINK:HREF'],10));
	
			if (!$imgData && $this->filesExt == 'sxw') {
					return $imgData = $this->unzipObj->getFileFromXML(substr($subV['attributes']['XLINK:HREF'],1)); 
			} elseif (!$imgData && $this->filesExt == 'odt') {
					return $imgData = $this->unzipObj->getFileFromXML(substr($subV['attributes']['XLINK:HREF'],9));
			}
